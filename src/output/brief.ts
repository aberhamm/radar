...
export function renderBrief(
  ...
  const totalFindings = scorecard.categories.reduce((sum, c) => sum + c.findings.length, 0);
  const riskCount = scorecard.topRisks.length;
  const findingsSummary = riskCount > 0
    ? `${totalFindings} findings, ${riskCount} risk${riskCount === 1 ? '' : 's'}`
    : `${totalFindings} findings, no risks`;
  lines.push(`**Investigation:** ${toolCallsUsed} tool calls | **Findings:** ${findingsSummary}`);
  lines.push('');
  lines.push('---');
  lines.push('');
