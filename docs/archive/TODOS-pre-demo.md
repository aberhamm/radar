...
- [x] **Marketing positioning doc** — `docs/radar-positioning.md`. 1-page business-language doc: what Radar does, by-the-numbers from gauntlet, engagement lifecycle fit, assessment types, technical details. **P2** — done 2026-04-13.
...
- [ ] **SSR pre-fetch for shared run URLs** — When a user opens `/run/{id}` in a new tab, run data loads client-side after hydration. Could pre-fetch via server component for faster first paint. Low priority since the SPA shell loads fast and the fetch is quick. **P3** — optimization.
- [ ] **Full App Router migration** — Current architecture uses catch-all `[[...slug]]` + pushState. Could migrate to proper Next.js App Router with separate route segments (`/run/[id]/page.tsx`, `/compare/[a]/[b]/page.tsx`). Deferred because SSE connection teardown/reconnect on route transitions makes this a bigger lift. Revisit if the SPA approach becomes limiting. **P3** — architecture.
...
**UNRESOLVED:** Keyboard nav for findings/sidebar (P2) **VERDICT:** ENG + DESIGN CLEARED for April 26 demo. Navigation architecture shipped (URL routing, sidebar nav, cross-linking, breadcrumbs). Fixed: border-l anti-pattern, flat nav compliance, dark mode colors, ARIA attributes.

other todos:

- ActivityChipGroup needs more v padding around tools
- replay happens too fast and doesnt look real
- `files examined` section doesnt need to scroll, but should expand on child files added
