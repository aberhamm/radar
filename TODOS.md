...
2. **Quick smoke test**: `npx tsx src/index.ts --help` or `npx tsx src/index.ts tools` — should list 22 tools
...
- [ ] **fetch_url redirect safety** — Cross-host redirects follow blindly. On AWS/Bedrock this is a real SSRF vector (`169.254.169.254` metadata endpoint). Return the redirect URL instead of following cross-host. Cap at 10 hops. `src/tools/web/fetchUrl.ts`. (P0, XS effort) **SECURITY**
