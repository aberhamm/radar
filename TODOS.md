...
- [ ] **Wire XP migration reference into goal-migration rules** — `goal-migration.md` currently assumes JavaScript/Next.js version gaps only. Add instruction for agent to `load_reference('sitecore/xp-migration-readiness')` when Sitecore XP is detected. Add XP-specific scorecard categories: "Platform Support & Licensing," "XP-Only Capabilities," "Custom Code Inventory," "Content & Data Migration." File: `src/rules/goal-migration.md`.
- [ ] **Sitecore XP specialist checklist** — New `src/rules/specialists/cms-sitecore-xp.md` covering: xConnect pipeline health, EXM configuration, custom processors, Solr index setup, session state management, publishing targets, security hardening (admin pages, patch priority), serialization strategy (Unicorn vs SCS vs TDS). Each item verifiable from code/config. Validate against a real XP repo before merging.
- [ ] **XP reference files** — `xp-migration-readiness.md` already exists (created 2026-05-14). Still needed: `src/references/sitecore/xp-common-antipatterns.md` (hardcoded connection strings, missing patch priority, oversized indexes, custom processors without error handling), `src/references/sitecore/xp-infrastructure-patterns.md` (topology, scaling, session state, Solr architecture).
